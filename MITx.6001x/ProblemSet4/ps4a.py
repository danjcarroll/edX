...
#
def getWordScore(word, HAND_SIZE):
    """
...
def displayHand(hand):
    ...
    for letter in hand.keys(): 
        for j in range(hand[letter]):   #to handle multiple instances of letter
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print ('"%s" earned %d points. Total: %d') % (word,getWordScore(word, HAND_SIZE),totalScore)
                print 
                # Update the hand
